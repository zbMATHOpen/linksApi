...
ns = api.namespace(
    "link",
    description="Operations related to linking to zbMATH"
)
...
link_create_arguments.add_argument(
    "link relation",
    type=str,
    required=True
)
...
class LinkItem(Resource):
    ...
    @api.doc(params={
        "zbl code": {
            "description": "Ex: 0171.38503"
        },
        "source identifier": {
            "description": "Ex: 11.14#I1.i1.p1"
        },
        "partner name": {
            "description": "Ex: DLMF"
        }
    ...
    def post(self):
        ...
        target_obj = ZBTarget.query.filter_by(
            zbl_code=zbl_val
        ).first()
        if not target_obj:
